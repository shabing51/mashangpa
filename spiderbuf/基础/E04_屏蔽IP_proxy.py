...
cookies = {
    ...
    "_ga_7B42BKG1QE": "GS2.1.s1775094572$o2$g1$t1775094606$j26$l0$h0"
}
# 先向上一层url路径发送请求拿到要爬取的目标页面的url，这里有五个
url = "https://spiderbuf.cn/web-scraping-practice/block-ip-proxy"
...
a_s = soup.select("nav > ul > li > a")[:-1]  # 类似列表的返回值  最后一个url是陷阱,访问可能会被封IP,所以切片 所以要分析页面与响应源代码关系
# print("a_s", a_s)
"""
...
headers_ = {
    ...
    "pragma": "no-cache",
    "priority": "u=0, i",
    "referer": "https://spiderbuf.cn/web-scraping-practice/block-ip-proxy/5f685274073b",
    "sec-ch-ua": "\"Chromium\";v=\"146\", \"Not-A.Brand\";v=\"24\", \"Microsoft Edge\";v=\"146\"",
    "sec-ch-ua-mobile": "?0",
    "sec-ch-ua-platform": "\"Windows\"",
    "sec-fetch-dest": "document",
...
# cookies_ = {
#     "_ga": "GA1.1.261017283.1775044945",
#     "_ga_7B42BKG1QE": "GS2.1.s1775094572$o2$g1$t1775097354$j27$l0$h0"
# }

# print(headers == headers_, cookies == cookies_)
def get_by_page_url(url):
    response = requests.get(url, headers = headers_)
    html_io = StringIO(response.text)
    df = pd.read_html(html_io)[0]
    return df

all_dfs = [get_by_page_url(url) for url in urls]
