...
# """
# cookies = {
#     "sessionid": "9yveo3m8spnh5xef56nvvp4o99ugxm7a",
# }
#
# sum_num = 0
# for page in range(1, 2):
#     getM_= execjs.compile(open("./q14.js", "r", encoding="utf-8").read()).call("getM")
#     print("execjs", getM_)
#     getM = subprocess.check_output(['node', r"./q14.js"], text=True)
#     print("subprocess", getM)
#     url = "https://www.mashangpa.com" + getM_
#     data = {
#         "page": page
#     }
...
#
#     print(response.json())
#     data_sum = sum(response.json()['current_array'])
#     print(data_sum)
#     sum_num += data_sum
#
# print(f'20页总和为：{sum_num}')

# 测试 subprocess 输出是否纯净
import subprocess

result = subprocess.check_output(['node', './q14.js'], text=True)
print(f"原始输出: {repr(result)}")
print(f"长度: {len(result)}")
print(f"strip后: {repr(result.strip())}")

# execjs 输出
import execjs
with open('./q14.js', 'r', encoding='utf-8') as f:
    ctx = execjs.compile(f.read())
sign = ctx.call("getM")
print(f"execjs输出: {repr(sign)}")